...
存储库分片之后还是无法避免特定存储库请求过多的问题，像 Gitee/Gitlab，并没有好的方案实施突破单一服务器并发极限，而 Github 使用三副本读写分离的 Spokes 方案能够最多提供 3倍于单一服务器的并发读取能力，但不支持并发写入存储库。引入并发写入存储库并不是好的选择，这可能会带来更多的数据冲突，破坏一致性。
