...
资源的分片和请求的路由相伴而生，将存储库存储到不同服务器上后，则需要在这些服务器上实现对应的服务支持前端的请求，而前端也需要实现特定的路由机制，关于 Gitee 的路由机制架构，可以参考相关公开活动的演讲或者相关博客披露。Gitee 存储服务器上使用了 git-srv 作为 Git 传输协议后端服务，而 Github 则使用了 DGit/Spokes，Gitlab 使用了 Gitaly。不同平台的技术各有侧重，并不相同。而 Gitee 目前不足之处是存储服务器上依然安装了 Gitlab，没有完全剥离，实际上 Gitaly 也有 Ruby 代码实现存储库读写。与 Gitee 类似，Gitea 还有另一种方案，即将 Gitea 部署到多个服务器上共用 DB 支持分片，比如 [gitea.com](https://gitea.com) 便是这样的平台，但 gitea.com 似乎并不支持 SSH，因此并不能算有效的分片。

随着服务器的增加，还会对部署运维带来新的需求，这也是平台伸缩性的一部分。
