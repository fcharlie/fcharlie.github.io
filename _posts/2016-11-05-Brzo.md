...
很大的不同.为了支撑更大的用户规模,码云也在不断的改进,而本文也主要介绍码云分布式 Brzo GIT HTTP
服务器的开发经验.
...
过 NGINX 模块开发,于是, 笔者便参与到 NGINX 动态代理模块的开发.先后有 zouqilin 和 lowkey2046 参与其中.
...
这个时候,唯一没有分布式支持的就是 SVN 协议,作为 SVN 兼容实现的开发者,我在接受 SVN 分布式任后,使用 Boost.Asio 开发实现了
